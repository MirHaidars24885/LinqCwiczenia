...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var result = Emps.Select(emp => new { Surname = emp.Ename, Job = emp.Job });
            return result;
        }
        ...
        public static IEnumerable<object> Task7()
        {
            var result = Emps
                .GroupBy(emp => emp.Job)
                .Select(group => new { Job = group.Key, NumberOfEmployees = group.Count() });
            return result;
        }
        ...
        {
            var customObject = new { Ename = "No value", Job = (string)null, HireDate = (DateTime?)null };
            var result = Emps.Select(emp => new { emp.Ename, emp.Job, emp.HireDate })
                .Union(new[] { customObject });
            return result;
        }
       ...
       public static int Task13(int[] arr)
{
    ...
    return result;
        ...
        {
            var result = Emps.Select(emp => new { emp.Empno, emp.Ename, emp.Job, emp.Salary, emp.Deptno })
                .Concat<object>(Depts.Select(dept => new { dept.Deptno, dept.Dname, dept.Loc }));
            return result;
        }
